...
def beer_lambert(img, depth, beta=0.05):
    """Apply Beer-Lambert law for underwater light attenuation."""
    arr = np.array(img).astype(np.float32) / 255.0
    d = np.array(depth).astype(np.float32) / 255.0
    
    actual_depth = 1.0 - d  # Invert depth representation since UDepth gives inverted depth
    actual_depth = actual_depth[..., None]
    
    transmission = np.exp(-beta * actual_depth)
    arr = arr * transmission + (1 - transmission) * np.array([0.6, 0.8, 0.9])
    
    return Image.fromarray(np.uint8(np.clip(arr * 255, 0, 255)))


# Generic augmentation 1
def color_cast(img, depth=None):
    """Apply underwater color cast with optional depth modulation."""
    arr = np.array(img).astype(np.float32)
    
    if depth is not None:
        d = np.array(depth).astype(np.float32) / 255.0
        depth_factor = 1.0 - d
        depth_factor = depth_factor[..., None]
        
        base_cast = np.array([0.8, 1.0, 1.2])
        cast_strength = 0.3 + 0.4 * depth_factor
        cast = base_cast[None, None, :] * cast_strength + (1 - cast_strength)
    else:
        cast = np.array([random.uniform(0.7, 1.0),
                         random.uniform(0.8, 1.2),
                         random.uniform(0.9, 1.3)])
    
    arr = np.clip(arr * cast, 0, 255)
    return Image.fromarray(arr.astype(np.uint8))


# Generic augmentation 2
def local_blur(img, depth=None):
    """Apply local blur with optional depth-based intensity."""
    arr = np.array(img)
    h, w, _ = arr.shape
    
    x, y = random.randint(0, max(1, w//2)), random.randint(0, max(1, h//2))
    patch_h, patch_w = min(h//4, h-y), min(w//4, w-x)
    if patch_h <= 0 or patch_w <= 0:
        return img
    
    patch = Image.fromarray(arr[y:y+patch_h, x:x+patch_w])
    
    if depth is not None:
        depth_patch = np.array(depth)[y:y+patch_h, x:x+patch_w]
        avg_depth = np.mean(depth_patch) / 255.0
        blur_radius = 1 + 3 * (1 - avg_depth)
        saturation = 0.3 + 0.4 * avg_depth
    else:
        blur_radius = 2
        saturation = 0.5
    ...
    arr[y:y+patch_h, x:x+patch_w] = np.array(patch)
    return Image.fromarray(arr)


def apply_augmentation(img, depth):
    """Apply underwater augmentation pipeline."""
    if random.random() < 0.7:
        aug = beer_lambert(img, depth, beta=0.02)
    else:
        aug = img

    if random.random() < 0.5:
        aug = color_cast(aug, depth)

    if random.random() < 0.3:
        aug = local_blur(aug, depth)

    return aug


class UnderwaterDataset(Dataset):
    def __init__(self, img_dir, depth_dir, transform=None, num_views=2):
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(self.num_views):
            aug = apply_augmentation(img, depth)
            if self.transform:
